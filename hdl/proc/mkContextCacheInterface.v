...
//
// On Mon Jul  1 14:35:52 EDT 2024
//
//
// Ports:
// Name                         I/O  size props
...
module mkContextCacheInterface(CLK,
  ...
  begin
    ...
    if (RST_N != `BSV_RESET_VALUE)
      if (EN_getMemReq) $display("[ ]   > [%x]", cacheL2$getToMem[537:512]);
    ...
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_connectL1InstL2)
	$display("[I] > [%b]", cacheI$getToMem[538]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_connectL1DataL2)
	$display("[D] > [%b]", cacheD$getToMem[538]);
    if (RST_N != `BSV_RESET_VALUE)
      if (cacheL2$RDY_putFromProc && reqQL2$EMPTY_N) $display("[ ]  >");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_distributeL2Resp) $display("[ ]  <");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_distributeL2Resp)
	$display("[dist] reqTypeQ.first: %b", reqTypeQ$D_OUT);
    if (RST_N != `BSV_RESET_VALUE)
